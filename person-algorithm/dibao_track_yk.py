...
def build_target_from_bbox(bbox, image_shape, config: FollowConfig):
    ...
    ratio = max(bbox_h / image_h, config.min_ratio)

    est_distance = config.ref_distance_m * (config.target_ratio / ratio)
    distance_error = est_distance - config.desired_distance_m
    forward = np.clip(distance_error * config.distance_gain,
                      -config.max_back_m, config.max_forward_m)

    offset = (cx - image_w / 2.0) / (image_w / 2.0)
    lateral = np.clip(offset * config.max_lateral_m,
                      -config.max_lateral_m, config.max_lateral_m)
