...
# load images###############################################################################
root_dir = r"C:\Users\Altair\Desktop\realsense_data"
date = "08_11_10_36"
images_ids = os.listdir(os.path.join(root_dir, "RGB", date))
# txt_list = os.path.join(root_dir, "list.txt")
# with open(txt_list, "r") as f:
#     images_ids = f.readlines()
i = datetime.datetime.now()
video_name = "../user/output/3dpose_video_demo_2_{}_{}_{}_{}.avi".format(i.year, i.month, i.day, i.hour)
...
for ids in images_ids:
    frame = cv2.imread(os.path.join(root_dir, "RGB", date, ids))
    depth_image = np.load(os.path.join(root_dir, "Depth", date, ids.replace(".jpg", ".npy")))
    # output = my_trace.get_smooth_output(my_trackModule, det_setting, frame)
    _, output = my_trackModule.get_output(frame, **det_setting)
    if len(output)>2:
        output = output[:-1]
    if len(output)!=0:
        ...
        try:
            ...
            print(1)
        kp_2d, kp_3d = my_3dkp_model.get_output(frame, bbox, root_depth_list)
        ...
        # cv2.imshow("depth", td_image)
        # cv2.imshow("win_depth", td_image)
        # cv2.imwrite(os.path.join(r"D:\project\arch\data_point_res", ids.strip()+".jpg"), frame)
        # cv2.imwrite(os.path.join(r"D:\project\arch\data_point_res", ids.strip() + "_d.jpg"), td_image)
