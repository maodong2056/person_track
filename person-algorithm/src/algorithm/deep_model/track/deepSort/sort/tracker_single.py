...
class Tracker:
    ...
    def predict_person(self):
        ...
        if self.track is not None:
            self.track.predict_person(self.kf)
    ...
    def update_person(self, detections):
        ...
        # Update track set.
        if len(matches) != 0:
            self.track.update_person(self.kf, detections[matches[0][1]])
        ...
        if len(matches) == 0 and len(unmatched_detections) != 0:
            ...
            for detection_idx in unmatched_detections:
                ...
                conf = det.body_confidence
                if conf > max_conf:
                    max_idx = detection_idx
            if self.track is None or self.track.is_deleted():
                self._initiate_track_person(detections[max_idx])
        if self.track is not None and self.track.is_deleted():
            self.track = None


        # Update distance metric.
        # active_targets = [t.track_id for t in self.tracks if t.is_confirmed()]
        active_targets = [self.track.track_id] if self.track is not None and\
                                         self.track.is_confirmed() else []
        features, targets = [], []
        # for track in self.tracks:
        #     if not track.is_confirmed():
        #         continue
        ...
        #     targets += [track.track_id for _ in track.features]
        #     track.features = []
        if self.track is not None and self.track.is_confirmed():
            features += self.track.features
            targets += [self.track.track_id for _ in self.track.features]
            self.track.features = []
        self.metric.partial_fit(
            np.asarray(features), np.asarray(targets), active_targets)
    ...
    def _match_person(self, detections):
        ...
        # unconfirmed_tracks = [
        #     i for i, t in enumerate(self.tracks) if not t.is_confirmed()]
        confirmed_tracks = [0] if self.track is not None and \
                                           self.track.is_confirmed() else []
        unconfirmed_tracks = [0] if self.track is not None and \
                                             not self.track.is_confirmed() else []

        # Associate confirmed tracks using appearance features.
        matches_a, unmatched_tracks_a, unmatched_detections = \
            linear_assignment.matching_cascade(
                gated_metric, self.metric.matching_threshold, self.max_age,
                [] if self.track is None else [self.track], detections, confirmed_tracks)
        ...
        matches_b, unmatched_tracks_b, unmatched_detections = \
            linear_assignment.min_cost_matching(
                iou_matching.iou_cost_person, self.max_iou_distance, [] if self.track is None else [self.track],
                detections, iou_track_candidates, unmatched_detections)
