...
class MultiLossFactory(nn.Module):
    def __init__(self, num_joints, num_stages, with_heatmaps_loss, heatmaps_loss_factor,
                 with_ae_loss, push_loss_factor, pull_loss_factor, ae_loss_type='exp'):
        super().__init__()
        # init check
        ...
        self.heatmaps_loss = \
            nn.ModuleList(
                [
                    ...
                    for with_heatmaps_loss in with_heatmaps_loss
                ]
            )
        ...
        self.ae_loss = \
            nn.ModuleList(
                [
                    AELoss(ae_loss_type) if with_ae_loss else None
                    for with_ae_loss in with_ae_loss
                ]
            )
