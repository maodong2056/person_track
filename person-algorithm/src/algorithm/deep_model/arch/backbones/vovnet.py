...
#         self.module_names.append(unit_name)
#         self.add_module(unit_name, nn.BatchNorm2d(out_channels))
#
#     def forward(self, x):
#         if self.with_modulated_dcn:
#             # offset conv
...
class VoVNet(nn.Module):
    ...
    def __init__(self, conv_body, input_ch=3, out_features=None):
        """
        Args:
        ...
        # Optionally freeze (requires_grad=False) parts of the backbones
    ...
    def _initialize_weights(self):
        ...
        # logging.info('VoVNet initialization finished.')
...
if __name__ == '__main__':
    model = VoVNet(conv_body="V-19-dw-eSE", input_ch=3, out_features=["stage5"])
    print(model)
    input = torch.randn(1, 3, 320, 320)
    # output = model(input)
    dummy_input = torch.autograd.Variable(torch.randn(1, 3, 320, 320))
    torch.onnx.export(model, dummy_input, r'D:\Refer\centermask2\model\VoVNet19_dw_eSE.onnx', verbose=True, opset_version=10)
