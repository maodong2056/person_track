...
class PersonFollow(BasicTask):
    def __init__(self, model_setting, person_item=PersonFollowItem):
        ...
        self.last_init_id = -1
        self.conform_int_max_number = 2#self.para["conform_int_max_number"]
        self.max_feature_bank = 6 # add近距离模板#self.para["max_feature_bank"]
        ...
        self.occ_feat_score = 0.32#self.para["occ_feat_score"]
        self.glob_feat_score = 0.3#self.para["glob_feat_score"]
        self.gate_threshold = 0.35
        self.target_id = []
        self.history_id = []
        self.lost_frame_count = 0
        self.max_lost_frame = self.para["max_lost_frame"]
        self.save_feat_score = 0.5#self.para["save_feat_score"]
        self.detindex = -1
    ...
    def conform_init_box(self, image, person_items):
        ...
        if ratio_w > 0.1 and ratio_w < 0.9 and ratio_h > 0.01 and ratio_h < 0.99 and ratio_box_h_img >= 0.001 :
    ...
    def init_track(self, image, person_items):
        ...
        if conforme_init_box_init > -1:
            ...
            if self.same_init_num_count > self.conform_int_max_number:
                ...
                if init_box[3] / image.shape[0] < 0.95:
        ...
        if confirm_state:
            ...
            if not occl_state and (self.track_count_num % self.max_save_frame)==0:
                ...
                if self.Init_featuer_active == False:
                    if bbox[3] / image.shape[0] < 0.95:#沁宝最近距离50cm露出脚开始保持第一帧模板
                ...
                # emb = self.__get_feaure(image, [target_person])
                    ...
                    if det_dist_score <= self.save_feat_score and det_box_index != -1:
                        ...
                        if bbox[0] / image.shape[1] >= 0.1 and bbox[2] / image.shape[1] <= 0.9 and  \
                         bbox[3] / image.shape[0] <=0.6 and ratio_h >=0.1:
                        ...
                        if bbox[1] / image.shape[0] <= 0.04 and bbox[3] / image.shape[0] <= 0.65:#增加近距离两个模板
                            print ('===jindist=====',bbox[3] / image.shape[0])

                            self.__update_feaure(self.tracking_person, det_feat, 3)

                        if bbox[1] / image.shape[0] >= 0.2 :#增加远距离全身模板，防止被更新掉
