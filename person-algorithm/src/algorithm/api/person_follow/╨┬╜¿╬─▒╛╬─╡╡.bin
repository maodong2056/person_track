...
class PersonFollow(BasicTask):
    def __init__(self, model_setting, person_item=PersonFollowItem):
        ...
        self.glob_feat_score = 0.22#self.para["glob_feat_score"]
        self.gate_threshold = 0.23
        self.target_id = []
        self.history_id = []
    ...
    def conform_init_box(self, image, person_items):
        ...
        for box_index, result in enumerate(person_items):
            ...
            box_center_dist =  abs(0.5 - box_center_w)
        ...
        # 重新加权，加上面积
        new_init_indexs = []
        for i in range(0,len(center_dist)):
            new_center = (1-center_dist[i])*box_area[i]
            new_init_indexs.append(new_center)

        min_dist_index = new_init_indexs.index(max(new_init_indexs))
        ...
        if confirm_state:
            ...
            if not occl_state and (self.track_count_num % self.max_save_frame)==0:
                ...
                if self.Init_featuer_active == False:
                    if bbox[3] / image.shape[0] < 0.925:#沁宝最近距离50cm露出脚开始保持第一帧模板
                ...
                # emb = self.__get_feaure(image, [target_person])
                    ...
                    if det_dist_score <= self.save_feat_score and det_box_index != -1:
                        ...
                        if bbox[0] / image.shape[1] >= 0.2 and bbox[2] / image.shape[1] <= 0.8 and  \
                                bbox[1] / image.shape[0] >= 0.03 and bbox[3] / image.shape[0] <=0.8 and ratio_h >=0.23:
                        ...
                        if bbox[1] / image.shape[0] <= 0.04 and bbox[3] / image.shape[0] <= 0.75:#增加近距离两个模板
