...
class RRClassifier:
    def __init__(self, alpha=1, sampleNumbers=64):
    ...
    def update_cache(self, features, labels):
        ...
        for i in range(len(labels)):
            ...
            if labels[i] == 1:
                ...
                #     if score_pos_st < 0.7:
                #         continue
                self.posTrainSet.append(features[i])
                ...
                self.negTrainSet.append(features[i])
...
class RRClassifierWithStrategy(RRClassifier):
    ...
    def getScore(self, query, reference):
        ...
        return score, self.scaledCosSimilarity(query[0], reference_np[:, 0]), abs(
            query[1] - sum(reference_np[:, 1]) / length), abs(query[2] - sum(reference_np[:, 2]) / length), abs(
            query[3] - sum(reference_np[:, 3]) / length)
    ...
    def update_cache(self, features, labels):
        ...
        for i in range(len(features)):
            ...
            if labels[i] == 1:
                ...
                print('=====pos feature====',len(self.posTrainSet_st))
            ...
            if len(self.posTrainSet_lt) > self.samples_st:
                sampleNums = int((self.samples - self.samples_st) / (self.samples_lt - self.samples_st) * len(
                    self.posTrainSet_lt[:(self.samples_lt - self.samples_st)]))
                self.posTrainSet = random.sample(self.posTrainSet_lt[:(self.samples_lt - self.samples_st)],
                                                 sampleNums) + self.posTrainSet_st
                self.posLabels = random.sample(self.posLabels_lt[:(self.samples_lt - self.samples_st)],
                                               sampleNums) + self.posLabels_st
            else:
            ...
            if len(self.negTrainSet_lt) > self.samples_st:
                sampleNums = int((self.samples - self.samples_st) / (self.samples_lt - self.samples_st) * len(
                    self.negTrainSet_lt[:(self.samples_lt - self.samples_st)]))
                self.negTrainSet = random.sample(self.negTrainSet_lt[:(self.samples_lt - self.samples_st)],
                                                 sampleNums) + self.negTrainSet_st
                self.negLabels = random.sample(self.negLabels_lt[:(self.samples_lt - self.samples_st)],
                                               sampleNums) + self.negLabels_st
            else:
    ...
    def predict(self, feature):
        # print( '====knnpre===',self.clf.predict_proba([feature]))
        # prepro = self.clf.predict_proba([feature])
        # print('prob',prepro)
        # return self.clf.predict_proba([feature])[0][1]
        return self.clf.predict([feature])[0]
