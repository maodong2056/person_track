...
class GraFormer(nn.Module):
    def __init__(self, edges, hid_dim=128, coords_dim=(2, 3), num_layers=4,
                 n_head=4,  dropout=0.1, n_pts=21, device=torch.device("cuda")):
        super(GraFormer, self).__init__()
        self.n_layers = num_layers
        self.device = device
        self.adj = adj_mx_from_edges(num_pts=n_pts, edges=edges, sparse=False).to(self.device)
