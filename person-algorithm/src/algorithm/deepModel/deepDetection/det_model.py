...
from src.algorithm.baseAlgorithm import min_cost_matching_tag, min_cost_matching_openpose, tag_cost
...
class DeepDetModelOpenPose(DeepDetModel):
    ...
    def face_postprocess(self, fhm, freg, fwh, flm, ratios ,dw,dh ,tag, thresh=0.3):
        ...
        for cls, reg, wh, flm, tag in zip(fclses, fregs, fwhs, flms, tags):
            ...
            flm0 = ((flm_c[0] + ctx) * 4 - dw) / ratio[1]
            flm1 = ((flm_c[1] + cty) * 4 - dh) / ratio[0]
            flm2 = ((flm_c[2] + ctx) * 4 - dw) / ratio[1]
            flm3 = ((flm_c[3] + cty) * 4 - dh) / ratio[0]
            flm4 = ((flm_c[4] + ctx) * 4 - dw) / ratio[1]
            flm5 = ((flm_c[5] + cty) * 4 - dh) / ratio[0]
            flm6 = ((flm_c[6] + ctx) * 4 - dw) / ratio[1]
            flm7 = ((flm_c[7] + cty) * 4 - dh) / ratio[0]
            flm8 = ((flm_c[8] + ctx) * 4 - dw) / ratio[1]
            flm9 = ((flm_c[9] + cty) * 4 - dh) / ratio[0]
