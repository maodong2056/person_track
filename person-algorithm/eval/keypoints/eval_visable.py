    ...
    def eval(self):
        ...
        for img_id in range(len(self.gt_bboxes)):
            ...
            if img_id not in self.pr_bboxes.keys():
                img = self.draw_hkp(img, np.array(self.gt_kpts[img_id]), 0)  # draw ground truth
            ...
            if self.write_image:
                img = self.draw_hkp(img, np.array(matches_kps), 1)
                img = self.draw_hkp(img, np.array(false_kps), 2)
                cv2.imwrite(os.path.join(self.vis_dir, file_name), img)
